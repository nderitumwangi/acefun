...
import {MPESA_PAYMENT_URL, RAZORPAY_PAYMENT_URL, MPESA_STK_PUSH_QUERY_URL} from "../../../../configs";
...
class PaymentList extends Component {
    ...
    __handleMpesa = () => {
        let self = this;
        this.setState({m_pesa_opened: true});
        const totalAmount = formatPrice(parseFloat(this.getTotalAfterCalculation()));

        Axios.post(MPESA_PAYMENT_URL, {
            totalAmount: totalAmount,
        })
            .then((res) => {
                ...
                if (res.data.status === 200) {
                    ...
                    // get request id
                    const CheckoutRequestID = JSON.parse(res.data.data).CheckoutRequestID;

                    let timer = setInterval(function () {
                        Axios.post(MPESA_STK_PUSH_QUERY_URL, {checkoutRequestId: CheckoutRequestID}).then((res) => {
                            if (res.data.result_code === 0) {
                                self.setState({m_pesa_opened: false, m_pesa_success: true});
                                const payment_token = "";
                                const method = "M-PESA";
                                self.__placeOrder(payment_token, method);
                                clearInterval(timer)
                                        ...
                                        <div className="col-12 paymentGatewayBlock">
                                            {gateway.name === "Stripe" && (
                                                <StripeCheckout
                                                    ...
                                                    ComponentClass="div"
                                                    image={`${window.location.origin.toString()}/assets/img/favicons/${localStorage.getItem(
                                                        "favicon-128x128"
                                                    )}`}
                                            ...
                                            >
                                                <div className="block block-link-shadow text-left shadow-light">
                                                    <div
                                                        ...
                                                        <div className="float-right mt-10">
                                                            <img
                                                                src="/assets/img/various/mpesa.png"
                                                                alt={gateway.name}
                                                                className="img-fluid"
                                                            />
                                                        </div>
                                                        ...
                                                        <div className="font-size-sm font-w600 text-muted">
                                                            You will get a notification on your phone to enter your
                                                            M-Pesa pin.
                                                        </div>
                                                    </div>
                                                </div>
