...
@section('content')
        ...
        <form action="{{ route('restaurant.post.searchItems') }}" method="GET">
            <div class="form-group form-group-feedback form-group-feedback-right search-box">
                <input type="text" class="form-control form-control-lg search-input"
                       placeholder="{{__('storeDashboard.ipSearchPH')}}"
                       name="query">
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @foreach ($items as $item)
                            <tr>
                                <td><img src="{{substr(url("/"), 0, strrpos(url("/"), '/'))}}{{ $item->item->image }}"
                                         alt="{{ $item->item->name }}" height="80" width="80"
                                         style="border-radius: 0.275rem;">
                                </td>
                                <td>{{ $item->item->name }}</td>
                                <td>{{ $item->restaurant->name }}</td>
                                <td>{{ $item->item->item_category->name }}</td>
                                <td>{{ $item->price }}</td>
                                <td>
                                        ...
                                        <div class="checkbox checkbox-switchery ml-1" style="padding-top: 0.8rem;">
                                            <label>
                                                <input value="true" type="checkbox" class="action-switch"
                                                       @if($item->is_active) checked="checked"
                ...
                <div class="modal-body">
                    <form action="{{ route('restaurant.saveNewItem') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-9">
                                <input type="text" class="form-control form-control-lg item-name" name="name"
                                       placeholder="Item Name"
                                       required oninput="searchItem(this.value)">
                                <button onclick="clearInputs()" class="mt-2 btn btn-primary clear-form-button d-none">
                                    Reset Form
                                </button>
                                <input type="text" class="form-control form-control-lg temp-item d-none"
                                       name="temp_item"
                                       placeholder="Item Name">
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-9 card search-results d-none">
                        ...
                        <div class="form-group row" style="display: none;" id="discountedTwoPrice">
                            ...
                            <div class="col-lg-6">
                                <label class="col-form-label"><span
                                            class="text-danger">*</span>{{__('storeDashboard.ipmLabelSellingPrice')}}
                                    :</label>
                        ...
                        <div class="form-group row">
                            <label class="col-lg-3 col-form-label"><span
                                        class="text-danger">*</span>{{__('storeDashboard.ipmLabelItemRestaurant')}}
                                :</label>
                            <div class="col-lg-9">
                                <select class="form-control select-search select" name="restaurant_id" required>
                                    @foreach ($restaurants as $restaurant)
                                        <option value="{{ $restaurant->id }}"
                                                class="text-capitalize">{{ $restaurant->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group row d-none">
                            <label class="col-lg-3 col-form-label"><span
                                        class="text-danger">*</span>{{__('storeDashboard.ipmLabelItemCategory')}}
                                :</label>
                            <div class="col-lg-9">
                                <select class="form-control select-search category_id" name="item_category_id">
                        ...
                        <div class="form-group row d-none">
                            <label class="col-lg-3 col-form-label">{{__('storeDashboard.ipmLabelAddonCategory')}}
                                :</label>
                            <div class="col-lg-9">
                        ...
                        <div class="form-group row d-none">
                            <label class="col-lg-3 col-form-label"><span
                                        class="text-danger">*</span>{{__('storeDashboard.ipmLabelImage')}}:</label>
                            <div class="col-lg-9">
                                <img class="slider-preview-image hidden"/>
                                <div class="uploader">
                                    <input type="file" class="form-control-lg form-control-uniform item-image"
                                           name="image"
    ...
    <div id="addBulkItemModal" class="modal fade mt-5" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><span class="font-weight-bold">{{__('storeDashboard.ipmCsvTitle')}}</span>
                    </h5>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
    ...
    <script>
        function searchItem(val) {
            $.ajax({
                type: 'GET',
                url: "{{url('/api/temp-items/search/')}}/" + val,
                success: function (data) {
                    $('.search-results').html(data)
                    $('.search-results').addClass('d-block').removeClass('d-none')
                    $('.clear-form-button').addClass('d-block').removeClass('d-none')
                },
                error: function (err) {
                    console.log(err)
                }
        ...
        function populateFields(item) {
            ...
            $('.temp-item').val(item.id)
            $('.category_id').val(item.item_category_id)
            $('.item-name').val(item.name)
            $('.search-results').addClass('d-none').removeClass('d-block')
        }

        function clearInputs() {
            document.getElementById("new-item-form").reset();
            $('.clear-form-button').removeClass('d-block').addClass('d-none')
        }
    </script>
@endsection